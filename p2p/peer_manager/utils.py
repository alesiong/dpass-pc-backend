...
from p2p.exceptions import UnknownCommandError, ECIESDecryptionError, \
    MultiplexerError
from p2p.log import get_logger
from p2p.multiplexer import Packet
from p2p.multiplexer.session import MultiplexedSession
...
from p2p.utils import rlpxcipher
...
class Peer(gevent.Greenlet):
    ...
    log = get_logger('peer_manager.utils.peer')
    ...
    def __init__(self, peermanager, connection, remote_pubkey=None):
        ...
        self.log.debug('peer init', peer=self)
    ...
    def ip_port(self):
        try:
            ...
            self.log.error('ip_port failed', e=e)
            raise e
    ...
    def connect_service(self, service):
        ...
        self.log.debug('registering protocol', protocol=protocol.name,
                       peer=self)
    ...
    def receive_hello(self, proto, version, client_version_string, capabilities,
        ...
        self.log.debug('received hello', proto=proto, version=version,
        ...
        agree = self.peermanager.on_hello_received(
            proto, version, client_version_string, capabilities, listen_port,
            remote_pubkey)
        if not agree:
            return
        ...
        self.log.debug('connecting services',
        ...
        for service in sorted(self.peermanager.wired_services,
            ...
            if proto.name in remote_services:
                if proto.version in remote_services[proto.name]:
                    ...
                    self.log.debug('wrong version', service=proto.name,
    ...
    def send_packet(self, packet):
        ...
        self.log.debug('send packet', cmd=protocol.cmd_by_id[packet.cmd_id],
    ...
    def protocol_cmd_id_from_packet(self, packet):
        ...
        raise UnknownCommandError(
            'no protocol for protocol id %s' % packet.protocol_id)

    def _handle_packet(self, packet):
        assert isinstance(packet, Packet)
        try:
            protocol, cmd_id = self.protocol_cmd_id_from_packet(packet)
            self.log.debug('recv packet', peer=self,
            ...
            self.log.error('received unknown cmd', peer=self, error=e,
                           packet=packet)
        except Exception as e:
            self.log.error('failed to handle packet', peer=self, error=e)
    ...
    def send(self, data):
        ...
        try:
            ...
            self.log.debug('wrote data', size=len(data), ts=time.time())
        except gevent.socket.error as e:
            self.log.error('write error', errno=e.errno, reason=e.strerror)
            self.report_error('write error %r' % e.strerror)
            self.stop()
        except gevent.socket.timeout:
            self.log.error('write timeout')
    ...
    def _run_ingress_message(self):
        self.log.debug('peer starting main loop')
        ...
        while not self.is_stopped:
            ...
            try:
                ...
                self.log.error('read error', errno=e.errno, reason=e.strerror,
                               peer=self)
            ...
            try:
                ...
                self.report_error('network error %s' % e.strerror)
                if e.errno in (
                        errno.ENETDOWN, errno.ECONNRESET, errno.ETIMEDOUT,
                    ...
                    break
            if imsg:
                self.log.debug('read data', ts=time.time(), size=len(imsg))
                try:
                    self.mux.add_message(imsg)
                except (rlpxcipher.RLPxSessionError, ECIESDecryptionError) as e:
                    self.log.error('rlpx session error', peer=self, error=e)
                    self.report_error('rlpx session error')
                    self.stop()
                except MultiplexerError as e:
                    self.log.error('multiplexer error', peer=self, error=e)
    ...
    def stop(self):
        if not self.is_stopped:
            try:
                ...
                self.log.debug('peer stopped', peer=self)
                ...
                self.log.error("failed to gracefully shutdown peer", error=e)
            finally:
    ...
    def check_if_dumb_remote(self):
        ...
        if not self.hello_received:
            self.report_error(
                'No hello in {} seconds'.format(self.dumb_remote_timeout))
            self.stop()
