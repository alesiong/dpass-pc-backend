...
log = get_logger('peer_manager.protocol')
...
class ConnectionMonitor(gevent.Greenlet):
    ...
    max_samples = 1000
    log = get_logger('peer_manager.utils.connection_monitor')
    ...
    def latency(self, num_samples=max_samples):
        ...
        return sum(self.samples[i] for i in
                   range(num_samples)) / num_samples if num_samples else 1
    ...
    def _run(self):
        ...
        while True:
            ...
            self.log.debug('latency', peer=self.proto,
...
class P2PProtocol(BaseProtocol):
    ...
    class hello(BaseProtocol.command):
        ...
        structure = [
            ...
            ('capabilities', sedes.CountableList(
        ...
        def create(self, proto):
            return dict(version=proto.version,
                        client_version_string=proto.config[
                            'client_version_string'],
                        capabilities=proto.peer.capabilities,
                        listen_port=proto.config['listen_port'],
    ...
    def get_hello_packet(cls, peer):
        ...
        res = dict(version=cls.version,
                   ...
                   listen_port=peer.config['listen_port'],
    ...
    class disconnect(BaseProtocol.command):
        ...
        def create(self, proto, reason=reason.client_quitting):
            ...
            log.debug('send_disconnect', peer=proto.peer,
                      reason=self.reason_name(reason))
            proto.peer.report_error(
                'sending disconnect %s' % self.reason_name(reason))
