...
from p2p.utils import int_to_big_endian
from p2p.utils.crypto import sha3, ecdsa_recover, ECCx


def sha3_256(x):
    return SHA3_256.new(update_after_digest=True, data=x)
...
class RLPxSession(object):
    ...
    def encrypt(self, header, frame):
        ...
        def aes(data=''):
            return self.aes_enc.encrypt(data)
    ...
    def decrypt_body(self, data, body_size):
        ...
        expected_frame_mac = mac(
            sxor(self.mac_enc(mac()[:16]), fmac_seed[:16]))[:16]
    ...
    def create_auth_message(self, remote_pubkey, ephemeral_privkey=None,
                            nonce=None):
        """
        ...
        self.initiator_nonce = nonce or sha3(
            int_to_big_endian(random.randint(0, 2 ** 256 - 1)))
    ...
    def create_auth_ack_message(self, version=supported_rlpx_version,
                                eip8=False, ephemeral_pubkey=None, nonce=None):
        """
    ...
    def encrypt_auth_ack_message(self, ack_message, eip8=False,
                                 remote_pubkey=None):
        assert not self.is_initiator
        remote_pubkey = remote_pubkey or self.remote_pubkey
        if eip8 or self.got_eip8_auth:
            # The EIP-8 version has an authenticated length prefix.
            prefix = struct.pack('>H', len(
                ack_message) + self.ecc.ecies_encrypt_overhead_length)
    ...
    def decode_auth_ack_message(self, ciphertext):
        ...
        try:
            (size, eph_pubkey, nonce, version) = self.decode_ack_plain(
                ciphertext)
        except AuthenticationError:
            (size, eph_pubkey, nonce, version) = self.decode_ack_eip8(
                ciphertext)
    ...
    def setup_cipher(self):
        ...
        shared_secret = sha3(
            ecdhe_shared_secret + sha3(
                self.responder_nonce + self.initiator_nonce))
        ...
        # iv = "\x00" * 16
        # assert len(iv) == 16
        # self.aes_enc = pyelliptic.Cipher(self.aes_secret, iv, 1,
        #                                  ciphername=ciphername)
        # self.aes_dec = pyelliptic.Cipher(self.aes_secret, iv, 0,
        #                                  ciphername=ciphername)

        self.aes_enc = AES.new(self.aes_secret, AES.MODE_CTR, nonce=b'\x00')
        self.aes_dec = AES.new(self.aes_secret, AES.MODE_CTR, nonce=b'\x00')
