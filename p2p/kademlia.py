...
from p2p.utils.crypto import sha3
from p2p.utils import big_endian_to_int
...
#     """
#
#     def __init__(self, proto, targetid, via_node=None,
#                  timeout=k_request_timeout, callback=None):
...
class KademliaProtocol(object):
    ...
    def update(self, node, pingid=None):
                ...
                for key in self._expected_pongs:
                    if key.endswith(node.pubkey):
                        log.debug(
                            ...
                            node=node,
                            expected_echo=encode_hex(
                                key[:len(node.pubkey)][:8]),
                            received_echo=encode_hex(
                                pingid[:len(node.pubkey)][:8]))
            return
        ...
        if pingid in self._expected_pongs:
            ...
            if replacement:
                ...
                self.routing.bucket_by_node(
                    replacement).replacement_cache.append(replacement)
            del self._expected_pongs[pingid]

        # add node
        eviction_candidate = self.routing.add_node(node)
        if eviction_candidate:
            log.debug('could not add', remoteid=node,
                      pinging=eviction_candidate)
    ...
    def recv_ping(self, remote, echo):
        ...
        if remote == self.this_node:
            log.warning('recv ping from self?!')
            return
        self.update(remote)
    ...
    def recv_pong(self, remote, echoed):
        ...
        log.debug('recv pong', remote=remote, pingid=encode_hex(pingid)[:8],
    ...
    def recv_neighbours(self, remote, neighbours):
        ...
        log.debug('recv neighbours', remoteid=remote, num=len(neighbours),
                  local=self.this_node,
            ...
            if time.time() < timeout:
                ...
                for close_node in closest[:k_find_concurrency]:
                    if not closest_known or \
                            close_node.id_distance(
                                nodeid) < closest_known.id_distance(nodeid):
