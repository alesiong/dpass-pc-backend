...
from p2p.exceptions import DeserializationError, MultiplexerError
...
class FrameCipherBase(object):
    ...
    def decrypt_body(self, data, body_size):
        assert len(
            data) >= self.header_len + self.mac_len + body_size + self.mac_len
...
class Frame(object):
    ...
    def frame_size(self):
        ...
        return self.header_size + self.mac_size + self.body_size(
            padded=True) + self.mac_size
    ...
    def header(self):
        ...
        assert tuple(l) == rlp.decode(header_data, sedes=header_data_sedes,
                                      strict=False)
...
class Multiplexer(object):
    ...
    def is_active_protocol(self, protocol_id):
        return True if sum(
            q.qsize() for q in self.queues[protocol_id].values()) else False
    ...
    def add_packet(self, packet):
        ...
        frames = Frame(packet.protocol_id, packet.cmd_id, packet.payload,
                       sequence_id=sid,
                       window_size=self.protocol_window_size(
                           packet.protocol_id),
    ...
    def decode_body(self, buffer, header=None):
        ...
        if self.frame_cipher:
            body = self.frame_cipher.decrypt_body(
                buffer[Frame.header_size + Frame.mac_size:].tobytes(),
                body_size)
            assert len(body) == body_size
            bytes_read = Frame.header_size + Frame.mac_size + ceil16(
                len(body)) + Frame.mac_size
        else:
            # header: frame-size || header-data || padding
            header = buffer[:Frame.header_size].tobytes()
            # frame-size: 3-byte integer size of frame, big endian encoded (excludes padding)
        ...
        # chunked-0: rlp.list(protocol-type, sequence-id, total-packet-size)
        try:
            header_data = rlp.decode(header[3:], sedes=header_data_sedes,
                                     strict=False)
        ...
        if sequence_id in chunkbuf:
            ...
            if chunked_0:
                raise MultiplexerError(
                    'received chunked_0 frame for existing buffer %d of protocol %d' %
                    (sequence_id, protocol_id))
            if len(body) > packet.total_payload_size - len(packet.payload):
                raise MultiplexerError(
                    'too much data for chunked buffer %d of protocol %d' %
                    (sequence_id, protocol_id))
            # all good
            packet.payload += body
            ...
            packet = Packet(protocol_id=protocol_id, cmd_id=cmd_id,
                            payload=payload)
            if chunked_0:
                if total_payload_size < len(payload):
                    raise MultiplexerError(
                        'total payload size smaller than initial chunk')
                if total_payload_size == len(payload):
    ...
    def decode(self, data=''):
        ...
        if not self._cached_decode_header:
            if len(self._decode_buffer) < Frame.header_size + Frame.mac_size:
                ...
                self._cached_decode_header = self.decode_header(
                    memoryview(self._decode_buffer))
                assert isinstance(self._cached_decode_header, bytes)

        body_size = \
            struct.unpack('>I', b'\x00' + self._cached_decode_header[:3])[0]
        required_len = Frame.header_size + Frame.mac_size + ceil16(
            body_size) + Frame.mac_size
        if len(self._decode_buffer) >= required_len:
            packet = self.decode_body(memoryview(self._decode_buffer),
                                      self._cached_decode_header)
