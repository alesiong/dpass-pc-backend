...
from p2p.exceptions import ProtocolError
...
class BaseProtocol(gevent.Greenlet):
    ...
    class command(object):
        ...
        def create(self, proto, *args, **kargs):
            ...
            assert not (kargs and isinstance(self.structure,
                                             sedes.CountableList))
        ...
        def encode_payload(cls, data):
            ...
            if isinstance(cls.structure, sedes.CountableList):
                ...
                return rlp.encode(data, sedes=sedes.List(
                    [x[1] for x in cls.structure]))
        ...
        def decode_payload(cls, rlp_data):
            ...
            if isinstance(cls.structure, sedes.CountableList):
                ...
                return dict(
                    (cls.structure[i][0], v) for i, v in enumerate(data))
    ...
    def _setup(self):
        ...
        klasses = [k for k in self.__class__.__dict__.values()
                   if isinstance(k, type) and issubclass(k,
                                                         self.command) and k != self.command]
        ...
        def create_methods(klass):
            ...
            def receive(packet):
                ...
                instance.receive(proto=self,
                                 data=klass.decode_payload(packet.payload))
        ...
        self.cmd_by_id = dict(
            (klass.cmd_id, klass.__name__) for klass in klasses)
