...
def get_table():
    ...
    if not check_expire:
        ...
        if hidden:
            data, hmac = encrypt_and_authenticate(KeyLookupTable.query.filter_by(hidden=False).first().encode(),
                                                  binascii.unhexlify(key))
            return jsonify(data=base64.encodebytes(data).decode().strip(),
                           hmac=base64.encodebytes(hmac).decode().strip())
        else:
            data, hmac = encrypt_and_authenticate(KeyLookupTable.query.all().encode(),
                                                  binascii.unhexlify(key))
            return jsonify(data=base64.encodebytes(data).decode().strip(),
                           hmac=base64.encodebytes(hmac).decode().strip())
